...
range_kappa = linspace(1,2,p_k); % set range for kappa
range_phi = linspace(0,1,p_p);
data_save{1} = 0;
...
%          noise_flag = 'additive_multiplicative'; % multiply kappa by random number also additive
        ...
        D_mult      = 400;          % maginitude of multiplicative stochasitc noise
        ...
        data_save_smooth{k,j} = h_vec_smooth;
         ...
         z = z1test(h_vec_smooth(1:500:end));
         M_normal(k,j) = Z;
         M_smooth(k,j) = z;
    end
end