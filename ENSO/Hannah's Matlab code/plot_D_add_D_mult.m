...
p_k = 21; % points of kappa examined
p_p = 21; % points of phi examined

range_add = linspace(0,0.3,p_k); % set range for D_add
range_mult = linspace(0,400,p_p); % set range for D_mult
data_save{1} = 0;
data_save_smooth{1} = 0;

M_normal = zeros(p_k,p_p);
M_smooth = zeros(p_k,p_p);
...
for k = 1:p_k
    for j = 1:p_p
        ...
        D_add       = range_add(k);        % magnitude of additive stochastic noise
        D_mult      = range_mult(j);          % maginitude of multiplicative stochasitc noise
        ...
        data_save{k,j} = h_vec;
        data_save_smooth{k,j} = h_vec_smooth;

        if k>10 && j>10
            Z = z1test(h_vec(1:500:end));
            z = z1test(h_vec_smooth(1:800:end));
        else
            Z = z1test(h_vec(1:500:end));
            z = z1test(h_vec_smooth(1:600:end));
        end
         M_normal(k,j) = Z;
         M_smooth(k,j) = z;
    end
